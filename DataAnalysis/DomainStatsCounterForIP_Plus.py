...
def main(input_file, output_file, chunksize, ip_address, mode):
    total_counts = Counter()
    first_time_dict = {}
    last_time_dict = {}

    
    # Fuzzy Matching for IP
    if mode == 'fuzzy':
        pattern = re.compile(ip_address if "IP" == "IP" else domain_name)
        chunk = chunk[chunk['IP'].str.contains(pattern)]
